...
class SentenceCVAE(Model):
    ...
    def __init__(self, embedder: TextFieldEmbedder,
        ...
        self.latent2hidden = nn.Linear(self.latent_dim + self.label_size, self.hidden_dim)
    ...
    @staticmethod
    def kl_anneal_function(step, anneal_steps: int = 300):
        return min(1, step / anneal_steps)

    def forward(self, input_tokens: Dict[str, torch.Tensor],
                output_tokens: Dict[str, torch.Tensor],
                classes: torch.LongTensor) -> Dict[str, torch.Tensor]:
        ...
        class_one_hot = F.one_hot(classes, num_classes=self.label_size).float().to(self.device)
        ...
        self.step += 1
        loss = ent_loss + (self.kl_anneal_function(self.step, self.anneal_steps)) * kl_loss
    ...
    def generate(self, class_name: str, batch_size: int = 1, max_len: int = 50):
        z = torch.randn([batch_size, self.latent_dim]).to(self.device)
        classes = torch.tensor([self.vocab.get_token_index(class_name, "class_labels")] * batch_size,
                               dtype=torch.long).to(self.device)
        class_one_hot = F.one_hot(classes, num_classes=self.label_size).float()
        z = torch.cat([z, class_one_hot], dim=-1)
